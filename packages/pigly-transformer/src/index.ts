...
function visitNode(node: ts.Node, program: ts.Program) {
  ...
  if (isCallExpression(node, typeChecker)) {
    ...
    if (methodName === 'SymbolFor') {
      ...
      newargs.push(ts.createCall(ts.createIdentifier('Symbol.for'), [], [ts.createStringLiteral("FOO")]));

      for(let arg of oldargs){        
        newargs.push(arg);
      } 
      
      node.arguments = ts.createNodeArray<ts.Expression>(newargs);
    }
  }
  return node;
}
...
function createSymbolFor(type: ts.Type, typeChecker: ts.TypeChecker) {
  ...
  //create a very basic, but consistent hash of the interface type;
