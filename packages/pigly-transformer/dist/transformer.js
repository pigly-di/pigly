...
function getSymbolId(symbol) {
    const uid = symbol.name + "#" + hash(prefix + symbol.id);
...
function getImportForType(typeChecker, imports, type) {
            ...
            if (importClause && importClause.namedBindings && ts.isNamedImports(importClause.namedBindings)) {
                ...
                for (const element of elements) {
                    if (ts.isImportSpecifier(element)) {
                        const importType = typeChecker.getTypeFromTypeNode(element.name);
                        const paramType = typeChecker.getTypeFromTypeNode(type);
                        if (importType.id === paramType.id) {
                            return element;
                        }
                    }
                }
...
function getTypeExpression(node, sourceFile, typeChecker) {
    let expression;
    let isArray = false;
...
function getClassConstructorParameters(node, sourceFile, typeChecker) {
    const type = typeChecker.getTypeFromTypeNode(node);
    if (!type.symbol.members)
        return [];
    const params = [];
    type.symbol.members.forEach(val => {
        if (val.declarations.length === 0)
            return;
...
function getClassImplementedInterfaces(node, sourceFile, typeChecker) {
    const type = typeChecker.getTypeFromTypeNode(node);
...
function visitParametersNode(node, typeChecker) {
    ...
    const newable = node.arguments[numArgs - 1];
...
function bindToImplements(node, typeChecker) {
    const signature = typeChecker.getResolvedSignature(node);
    if (!signature) {
        return node;
    }
    ...
    const newable = node.arguments[0];
...
function isCallExpression(node, typeChecker) {
